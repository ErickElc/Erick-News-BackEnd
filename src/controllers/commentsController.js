...
class commentsController {
  ...
  static async createComment(req, res) {
    ...
    try {
      ...
      if (req.body.userId != authorization._id)
        return res.status(403).send("não foi possível criar esse comentário");
      else {
        await comment.save();
  ...
  static async deleteComment(req, res) {
    ...
    try {
      ...
      if (findUser.userId._id != authorization._id)
        return res.status(403).send("Não foi possível deletar esse comentário");
