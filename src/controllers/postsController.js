...
class PostController {
  ...
  static async cadastrarPost(req, res) {
    ...
    try {
      ...
      if (authorization._id != findAutor._id)
        return res.status(403).send("Não foi possível criar esse documento");
      await post.save();
  ...
  static async editarPost(req, res) {
    ...
    try {
      ...
      if (authorization._id != findPost.autor._id)
        return res.status(403).send("Não foi possível editar esse documento");
  ...
  static async deletePost(req, res) {
    ...
    try {
      ...
      if (authorization._id != findPost.autor._id)
        return res.status(403).send("Não foi possível deletar esse documento");
